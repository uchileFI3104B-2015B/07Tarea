...
def inicializa_v(v,N_steps):
    ...
    '''inicializo velocidad nula pata t=0 en todo el espacio x , incluyendo condiciones de borde con x=0 y x=1'''
...
h = 1/9. # h = (x_final-x_inicial) / (N_steps - 1)
...
presion(x,rho)  # funciona bien, entrega las presiones para x entre 0 y 1 para t=0 con las densidades rho de cada x respectivo


def calculos_r(p,x,t,v,c,rho):


        # c2[i] = (gamma)*p[i]/rho[i]  #arreglo dados los p y rho arreglos
        ...
        for i in range(0,N_steps):
            ...
            m = x[i]+(v[i]-c[i])*(t[i-1]-t[i])-(v[i]-c[i])*x[i-1]/(v[i-1]+c[i-1])
            b = 1+(c[i]-v[i])/(v[i-1]+c[i-1])
            x_r[i] = m/b


            d = t[i]*(v[i]-c[i])+x[i-1]-x[i]-t[i-1]*(v[i-1]+c[i-1])
...
v_solucion[0,:] = v.copy()
rho_solucion[0, :] = rho.copy()  # caso base : obtengo rho para t=0 para el arreglo de x entre 0 y 1
...
#for i in range(0, int(N_pasos_temporales), 5):
ax.plot(x, rho_solucion[0, :],'g') #distintos tiempos
...
#ax2.plot(x, v_solucion[0.2, :])
ax2.plot(x, rho_solucion[0.2, :],'g') #distintos tiempos
